...
async def generate_order(user_id:int, payload: dict = Depends(validate_access_token), session = Depends(get_session)):
	# fetch user details
...
async def fetch_order_id(user_id:int, payload: dict = Depends(validate_access_token), session = Depends(get_session)):
	# fetch order_id
